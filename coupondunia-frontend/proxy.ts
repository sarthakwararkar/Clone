...
export async function proxy(request: NextRequest) {
  try {
    ...
    // Safely check for required Supabase env variables to prevent crash on edge
    const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL
    const supabaseKey = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY

    if (!supabaseUrl || !supabaseKey) {
      console.warn('Warning: NEXT_PUBLIC_SUPABASE_URL or NEXT_PUBLIC_SUPABASE_ANON_KEY is missing in env. Skipping middleware auth checks.')
      return NextResponse.next()
    }
    ...
    console.error('Critical middleware exception handled safely:', err)
    return NextResponse.next()
  }
}
